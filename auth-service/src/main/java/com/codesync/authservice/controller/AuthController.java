/*
 * Code reader note: Exposes REST endpoints for registration, OTP verification, login,
 * profile management, password flows, search, and admin user actions.
 * Annotations used: @RestController makes the class a REST endpoint source,
 * @RequestMapping sets the /auth base path, @RequiredArgsConstructor injects services,
 * the mapping annotations (@GetMapping, @PostMapping, @PutMapping, @DeleteMapping)
 * define routes, @AuthenticationPrincipal injects the authenticated email,
 * and @PreAuthorize protects admin-only operations.
 */
...
public class AuthController {
    ...
    // ─── PLAN UPDATE (internal, called by payment-service) ────────────────────

    /**
     * Update a user's subscription plan after payment.
     * PUT /auth/plan  (called internally by payment-service)
     */
    @PutMapping("/plan")
    public ResponseEntity<UserProfileDto> updatePlan(@RequestBody java.util.Map<String, String> body) {
        ...
        return ResponseEntity.ok(authService.updatePlan(email, plan, expiry));
    }
}
