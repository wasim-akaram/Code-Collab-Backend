...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    long countByOwnerEmailAndArchivedFalse(String ownerEmail);
    long countByOwnerEmailAndVisibilityAndArchivedFalse(String ownerEmail, ProjectVisibility visibility);
    ...
    // ✅ Dashboard: projects owned by the user OR where user is a member
    @Query("SELECT DISTINCT p FROM Project p LEFT JOIN ProjectMember m ON p.id = m.projectId " +
           "WHERE p.archived = false AND (p.ownerEmail = :email OR m.userEmail = :email)")
    Page<Project> findUserOwnedOrMemberProjects(
